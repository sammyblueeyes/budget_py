...
''')
    ...
    def test_gen_monthly_due_date_with_1_day_range_at_end_of_feb_non_leap_year(self):
        ...
        due_dates = c.get_due_dates(end_of_feb, MONTHLY)
        self.assertEqual(len(due_dates), 1)
        self.assertEqual(due_dates[0], start_date)

        # Cashflow date on last day of February
        start_date = end_of_feb
        c = Cashflow(start_date=start_date)

        due_date = datetime(2014, 1, 28)
        due_dates = c.get_due_dates(due_date, MONTHLY)
        self.assertEqual(len(due_dates), 1)
        self.assertEqual(due_dates[0], start_date)

        due_date = datetime(2014, 1, 29)
        due_dates = c.get_due_dates(due_date, MONTHLY)
        self.assertEqual(len(due_dates), 1)
        self.assertEqual(due_dates[0], start_date)

        due_date = datetime(2014, 1, 30)
        due_dates = c.get_due_dates(due_date, MONTHLY)
        self.assertEqual(len(due_dates), 1)
        self.assertEqual(due_dates[0], start_date)
